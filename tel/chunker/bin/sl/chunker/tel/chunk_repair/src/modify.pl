...
#				$log->info("\n");
	...
	my $i=$index;
	$log->info("Children $i $index \n");
	@nodes=&get_children(0,$sent);
	for($k=0;@_rhs_->[0]->[$k]->{"chunk"} ne "";$k++)
	{
		$log->info("$i Node:$nodes[$i] Chunk:@_rhs_->[0]->[$k]->{\"chunk\"}\n");
		if($nodes[$i] eq "")
		{
			last;
		}
...
#	$log->info("enter-check \n");
	for($k=0;@temp_array_rhs1->[0]->{"fs"}->[$k]->{"word"} ne "";$k++)
	{
		...
		push(@array_fs,@temp_array_rhs1->[0]->{"fs"}->[$k]->{"suff"});
		...
		while($l<$num)
		{
			if($array_fs[$l]=~/\$/)
			{
				if($array_fs[$l]=~/(\$[a-zA-Z0-9]+)$/)
				{
					$array_fs[$l]="$var_hash{$1}";
				}
				elsif($array_fs[$l]=~/([^\$]*\.)?(\$[a-zA-Z0-9])\.(.*)?/)
				{
					my $temp1=$1;
					my $var1=$2;
					my $temp2=$3;
		...
		$_small_TREE_[$k+1][4]="<fs af='$array_fs[2],$array_fs[3],$array_fs[4],$array_fs[5],$array_fs[6],$array_fs[7],$array_fs[8],$array_fs[9]' name=$vv>";
	}
	$_small_TREE_[$k+1][2]="))";
...
sub add_children
{
	...
	foreach (@temp_array)
	{
		...
		while($l<$num)
		{
			if($array[$l]=~/\$/)
			{
				if($array[$l]=~/(\$[a-zA-Z0-9]+)$/)
				{
					$array[$l]="$var_hash{$1}";
				}
				elsif($array[$l]=~/([^\$]*\.)?(\$[a-zA-Z0-9])\.(.*)?/)
				{
					my $temp1=$1;
					my $var1=$2;
					my $temp2=$3;
...
#		@.$uu.[0];
		...
		{
			$log->info("guess $temp_hash{$uu} $uu \n");
			if($temp_hash{$uu} eq "NULL")
			{
#				$array[$l]="";
				@var[0]="";
			}
			elsif($temp_hash{$uu}=~/(\$[a-zA-Z0-9]+)?=>(.*)?$/){
 #                               	$var=$3;
					@var[0]=$var_list{$1};
#					$log->info("Val $l - $array[$l]\n");
#					$log->info("testing-1  $temp_hash{$uu} $uu @var[0] \n");
			}
			elsif($temp_hash{$uu}=~/(^\$[a-zA-Z0-9]+)$/)
			{
				$try=$1;
				@var[0]=$var_hash{$1};
				$log->info("testing  $uu @var[0] \n");
...
#			}
			elsif($temp_hash{$uu}=~/([^\$]*\.)?(\$[a-zA-Z0-9])\.(.*)?/)
			{
				...
				my $var1=$2;
				my $temp2=$3;
#				$log->info("Vadsdsdl $l - $array[$l]\n");
#				$array[$l]=$temp1."$var_hash{$var1}".$temp2;
				if($temp1 ne '' and $temp2 ne ''){
					@var[0]=$temp1.$var_hash{$var1}.$temp2;}
				if($temp1 eq '' and $temp2 ne ''){
					@var[0]='"'.$var_hash{$var1}.$temp2;}
				if($temp1 ne '' and $temp2 eq ''){
					@var[0]=$temp1.$var_hash{$var1}.'"' ;}
				if($temp1 eq '' and $temp2 eq ''){
					@var[0]='"'.$var_hash{$var1}.'"' ;}
			}
			else{
				@var[0]=$temp_hash{$uu};
				$log->info("batao-errir \n");
				}
#			&update_attr_val("cat", \@cat,$fs_head_array,$sent);}
...
#				@check=&get_values("tam",$fs_array);
			...
			if($uu eq "word")
			{
				&modify_field($node,2,@var[0],$sent);
			}
			if($uu eq "pos")
			{
				&modify_field($node,3,@var[0],$sent);
			}
			&update_attr_val($uu, \@var,$fs_head_array,$sent);
		}
