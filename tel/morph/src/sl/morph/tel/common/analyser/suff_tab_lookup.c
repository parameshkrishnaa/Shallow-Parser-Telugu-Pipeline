...
GDBM_FILE db;              /* GNU database manager includes dbm and ndb compatability and
...

  {
	...
	sprintf(logmsg,"word=%s|db_out=%s\n",db_tag.dptr,db_out.dptr);PRINT_LOG(log_file,logmsg);
...
/*if(db_out.dptr != '\0') changed for DOS */
	  ...
	  for(loop1=0;;)
	   {
         ...
         ans_ar[0][0] = '\0';
		sprintf(logmsg,"word=%s|ans_ar=%s|ans_ar1=%s|\n",word,ans_ar,ans_ar1);PRINT_LOG(log_file,logmsg);
 ...
 /*This function creates a specfic structure for the rootword,priority pdgm and
  ...
  return ans_ar1->suff;  //returns the morph word 
  }
