...
char morph[Morphsize];                  /* word */
{
  ...
  int ISNUMBER;                        /* isnumber for checking numeric value */
	...
	//PRINT_LOG(log_file, " checking whether the given word is Avy or not.\n");
	  ...
	  return(-3);    /* numeral */
	else 		/*** searching in avy file ***/
	   {
		...
		return (-1);
              	...
              	{   return (-2);} /* Neither a numeral not an avy */
 	    }
}
