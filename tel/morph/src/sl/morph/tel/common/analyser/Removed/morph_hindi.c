                                        ...
                                        aux_verb,pdgm */
{
	...
	int AVY_srch();
	char *program_name="morph_hindi.c";
        PRINT_LOG(log_file, "This is fun_morph()\n");
	
	pos = 0;
	sizeof_dummy = sizeof(dummy);
	for(loop1=0;loop1<Arraysize;loop1++)  /* intializing root,pdgm,cat to null */
        {
			 ...
			 for(l= morph_size;l>=0;l--)  	/* checks for l size >0 */
                         {
                	          ...
                	          /* This function concatenates the root word with the suffix */                                   get_suff(morph,suff,l);
