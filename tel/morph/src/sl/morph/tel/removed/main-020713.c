...
//	char rls1[1500][300], cc[200];
	...
	GDBM_FILE db_clit;
		...
		db_clit = gdbm_open ("/home/sptil/Nagaraju/telmorph/src/sl/morph/tel/clitic_tel", 512, GDBM_READER, 0666, 0);
        if (!db_clit)
        {
...
//  fpath  -- /home/teltamilmt/sampark/data_bin/sl/morph/tel/
	...
	if (!ispunct(c))
	{
		...
		sprintf(logmsg, "INFO: left word and right word after sandhi split,now in main |%s|%s|%s|%s|%d|%d|%d|",lft_word,rt_word,ans_ar_left[0].root,ans_ar_right[0].root,no_of_ans_left,no_of_ans_right,ret_status); PRINT_LOG(log_file, logmsg);
		//printf("ret_status=|%d|\n",ret_status);
		//int ret_clitic=0;
