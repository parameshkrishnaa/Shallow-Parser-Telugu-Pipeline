...
//	char rls[700][100], ss[100];
	...
	char san[15][50];
	char temp[15][50];
	char san_file[BigArray];
	strcpy(san_file,getenv("setu"));
	strcat(san_file, "/morph/data_bin/sl/morph/tel/sandhi_tel");
	GDBM_FILE db_san;
        datum key1,data1,nextkey1;

		//db_san = gdbm_open ("../../../../data_bin/sl/morph/tel/sandhi_tel", 512, GDBM_READER, 0666, 0);
		db_san = gdbm_open (san_file, 512, GDBM_READER, 0666, 0);
		//db_san = gdbm_open ("/var/www/html/calts/indian-morph/tel-morph/data_bin/sl/morph/tel/sandhi_tel", 512, GDBM_READER, 0666, 0);
        if (!db_san)
        {
...
//		printf ("File cannot be opened\n");
	...
	//rule_pool->rule =(struct sandhi_rule *) malloc (sizeof (struct sandhi_rule) *(size_of_rls -3 ));
	rule_pool->rule =(struct sandhi_rule *) malloc (sizeof (struct sandhi_rule) *(size_of_rls ));


	//for (j = 3; j <size_of_rls; j++)//for flat file
	for (j = 0; j <size_of_rls; j++)//for dbm file
	{
		for (i = 0; i < 10; i++)
		{
			strcpy (san[i], "\0");
		...
		strcpy (rule_pool->rule[j - 0].rl_no, san[0]);
		//printf("sano=%s\n",san[0]);
		strcpy (rule_pool->rule[j - 0].pat, san[1]);
		strcpy (rule_pool->rule[j - 0].br_pt, san[2]);
		strcpy (rule_pool->rule[j - 0].lft_dlt, san[3]);
		strcpy (rule_pool->rule[j - 0].lft_add, san[4]);
		strcpy (rule_pool->rule[j - 0].rt_dlt, san[5]);
		strcpy (rule_pool->rule[j - 0].rt_add, san[6]);
		strcpy (rule_pool->rule[j - 0].lft_mo_call, san[7]);
		strcpy (rule_pool->rule[j - 0].rt_mo_call, san[8]);
		strcpy (rule_pool->rule[j - 0].lft_cond, san[9]);
		strcpy (rule_pool->rule[j - 0].rt_cond, san[10]);



		if (strcmp (rule_pool->rule[j - 0].lft_add, "0") == 0)
			strcpy (rule_pool->rule[j - 0].lft_add, "\0");
		if (strcmp (rule_pool->rule[j - 0].rt_add, "0") == 0)
			strcpy (rule_pool->rule[j - 0].rt_add, "\0");
		if (strcmp (rule_pool->rule[j - 0].lft_cond, "0") == 0)
			strcpy (rule_pool->rule[j - 0].lft_cond, "\0");
		if (strcmp (rule_pool->rule[j - 0].rt_cond, "0") == 0)
			strcpy (rule_pool->rule[j - 0].rt_cond, "\0");
	}
}
...
//	char rls1[1500][300], cc[200];
	...
	char temp1[15][50];
	char cli_file[BigArray];
	strcpy(cli_file,getenv("setu"));
	strcat(cli_file, "/data_bin/sl/morph/tel/clitic_tel");
	//GDBM_FILE db_clit;
        datum key,data,nextkey;

		//db_clit = gdbm_open ("../../../../data_bin/sl/morph/tel/clitic_tel", 512, GDBM_READER, 0666, my_fatal);
		db_clit = gdbm_open (cli_file, 512, GDBM_READER, 0666, my_fatal);
		//db_clit = gdbm_open ("/var/www/html/calts/indian-morph/tel-morph/data_bin/sl/morph/tel/clitic_tel", 512, GDBM_READER, 0666, my_fatal);
        if (!db_clit)
        {
	...
	rule_pool1->rule1 =
		(struct clitic_rule *) malloc (sizeof (struct clitic_rule) *
				(size_of_rls1));
	//memset(cli, '\0', 50);
	//for (j=2;j<size_of_rls1 ;j++) //for flat file
	for (j=0;j<size_of_rls1 ;j++)	//for dbm file
	{
		for (i = 0; i < 5; i++)
		{
			//printf("cli[%d][%d]=%s\n",j,i,cli[i]);
...
/*	int i;
	...
	sprintf(logmsg, "INFO: fun_morph() returned |morph=|%s|root=|%s|off=|%d|pdg1=|%s|pdg2=|%s|pdg3=|%s|cat1=|%s|cat2=|%s|cat3=|%s|",morph, ans_ar[var].root, ans_ar[var].offset, ans_ar[var].pdgm,ans_ar[1].pdgm,ans_ar[2].pdgm,ans_ar[0].cat,ans_ar[1].cat,ans_ar[2].cat); PRINT_LOG(log_file, logmsg);
	//printf("root1=%s|root2=%s|root3=%s\n",ans_ar[0].cat,ans_ar[1].cat,ans_ar[2].cat);
	ret_clitic=0;
...
strcmp(suffix_5,"Ata")==0||strcmp(suffix_5,"ata")==0||strcmp(suffix_3,"aMta")==0){
		...
		//printf("Iam here|%s|\n",morph);
		//blocked//sprintf(logmsg,"INFO: Calling clitic-stripper left word before clitic split,now in main |%s|%s|%d|",clt_lft_word,ans_clt_left[0].root,no_of_clt_left);PRINT_LOG(log_file,logmsg);
		//blocked//ret_clitic=clitic_morph(morph,&clitic_pool,fp_pdgm,DBM_FLAG,db_dict,db_suff,lft_word,ans_clt_left,tam,&no_of_ans_clt);
		//ret_clitic=clitic_morph(morph,&clitic_pool,fp_pdgm,DBM_FLAG,db_dict,db_suff,clt_lft_word,ans_clt_left,tam,&no_of_ans_left);
		//blocked//sprintf(logmsg,"INFO: left word after clitic split,now in main |%s|%s|%d|ret_clitic=%d|",ans_clt_left,ans_clt_left[0].root,no_of_ans,ret_clitic);PRINT_LOG(log_file,logmsg);
...
//  fpath  -- /home/teltamilmt/sampark/data_bin/sl/morph/tel/
	...
	if (!ispunct(c))
	{
		...
		int ret_status=0;
		category[0]='\0';//for derived_word cat
...
// paramesh: NCR Values are introduced here 310112 ...Ending
	...
	strcpy(category,ans_ar_tmp[loop1].cat);	//Nagaraju for copying root word category
	...
	//printf("root=%s|ans_category=%s|cat=%s|tmp=%d|\n",ans_ar_tmp[loop1].root,ans_ar_tmp[loop1].cat,cat,loop1);
	char suff_pdg[10];	//Added to check if the word is analyzed from derivation
	strcpy(suff_pdg,ans_ar_tmp[loop1].pdgm+strlen(ans_ar_tmp[loop1].pdgm)-4);
	//printf("suff_pdg=|%s|\n",suff_pdg);
	...
	sprintf(logmsg, "INFO: Calling resufun_hori(),root=%s|cat=%s|g=%s|n=%s|p=%s|case=%s|cm=%s|tam=%s|", root, cat, g, n, p, kase, cm, tam ); PRINT_LOG(log_file, logmsg);
	//Foll lines added by Nagaraj for derivational words that contain more than one entry in dict
	//printf("cat=%s|cat1=%s|der_Cat=%s|\n",ans_ar[0].cat,ans_ar[1].cat,cat);
	if(strcmp(suff_pdg,"deri")==0) {
	 if(((!strcmp(ans_ar[0].cat,"n") && !strcmp(ans_ar[1].cat,"v")) || (!strcmp(ans_ar[0].cat,"v") && !strcmp(ans_ar[1].cat,"n"))) && !strcmp(cat,"v"))
        ...
        strcpy(category,"n");
        if(((!strcmp(ans_ar[0].cat,"adj") && !strcmp(ans_ar[1].cat,"v")) || (!strcmp(ans_ar[0].cat,"v") && !strcmp(ans_ar[1].cat,"adj"))) && !strcmp(cat,"v") )
        strcpy(category,"v");
        if(((!strcmp(ans_ar[0].cat,"adj") && !strcmp(ans_ar[1].cat,"n")&&!strcmp(ans_ar[2].cat,"v")) || (!strcmp(ans_ar[0].cat,"n") && !strcmp(ans_ar[1].cat,"adj")&&!strcmp(ans_ar[2].cat,"v"))) && !strcmp(cat,"v"))
        strcpy(category,"v");
	}
	//Get the offset category only if the pdgm contains "_deri"
	if(strcmp(suff_pdg,"deri")!=0) {
	strcpy(cat,"\0");
	}												                               
	...
	//strcat(feature_str, cat);
			...
			//memset(cat,'\0',strlen(cat));//Added by Nagaraju to delete derived cat for adj and adverbs
			 }
	}
	...
	char wrd1[15][50];
	//char wrd[50][100];
	 const char* setu_name = "setu";  // or any environment variable like "PATH", "USER", etc.
    	char* value = getenv(setu_name);
	const char* main_file_name = "/morph/data_src/sl/morph/tel/pc_data/derivation/main.p";
	// Allocate enough space for full path
	char main_file_path[1024];  // Ensure it's large enough

	// Combine env value + file name
	snprintf(main_file_path, sizeof(main_file_path), "%s%s", value, main_file_name);

	sprintf(logmsg, "INFO: Calling |main_file_path=%s|", main_file_path); PRINT_LOG(log_file, logmsg);
	fp1=fopen(main_file_path,"r");
	//fp1=fopen("../../../../data_src/sl/morph/tel/pc_data/derivation/main.p","r");
	//fp1=fopen("/var/www/html/calts/indian-morph/tel-morph/data_src/sl/morph/tel/pc_data/derivation/main.p","r");
	if(fp1!=NULL){
		//printf("main.p opned");
	...
	strcat(feature_str, p);
		memset(p, '\0', sizeof(p));
	strcat(feature_str, ",");
	//printf("Iam here with cm=%s|tam=%s\n",cm,tam);
	strcpy(cm,tam);
