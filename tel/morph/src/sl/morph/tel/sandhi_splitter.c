	...
	{				// for every rule in rule pool
		...
		if (pattern_matched[i] == 1) {
			...
			if(ans_ar_left[0].pdgm !='\0'){//&& ans_ar_left[0].offset==-2) 
				...
				//blocked//read_clitics ("../../../../data_src/sl/morph/tel/pc_data/clitics-rls", &clitic_pool);
				//blocked//sprintf(logmsg,"before cli_str in sandhi_splitter test_left=|%s|lft_word1=|%s|ans_ar_left[0].root=|%s|\n",test_left,lft_word1,ans_ar_left[0].root);PRINT_LOG(log_file,logmsg);
				//blocked//ret_clitic=clitic_morph(test_left,&clitic_pool,fp_pdgm,DBM_FLAG,db_lex,db_suff,lft_word1,ans_ar_left,tam,&no_of_ans_clt_lft);
				//blocked//sprintf(logmsg,"after cli_str in sandhi_splitter test_left=|%s|lft_word1=|%s|ans_ar_left[0].root=|%s|offset=|%d|\n",test_left,lft_word1,ans_ar_left[0].root,ans_ar_left[0].offset);PRINT_LOG(log_file,logmsg);
				if(ret_clitic == 1) {
			...
			sprintf(logmsg,"cat=%s|lft_cond=%s|catl=%s|rulecatl=%s|catr=%s|rulecatr=%s|",cat,rule_pool->rule[i].lft_cond,catl,rulecatl,catr,rulecatr);PRINT_LOG(log_file, logmsg);
			if( ( (strcmp(cat, rule_pool->rule[i].lft_cond)==0) || (strcmp(catl,rulecatl)==0) || (strcmp(catr,rulecatr)==0) )&& (strcmp(cat,",")!=0))   {	//standard condition is preferred
			//if( ( (strcmp(cat, rule_pool->rule[i].lft_cond)==0) || (strcmp(catl,rulecatl)==0) || (strcmp(catr,rulecatr)==0) ) && (strcmp(cat,",")==1))//blocked because standard condition is preferred Nagaraju 070813  
				...
				ret_clitic=0;
				int no_of_ans_clt_rgt;;
				char rgt_word1[50];
				struct ans_struct *ans_clt_rgt;
				//			sprintf(logmsg, "INFO: answer array after fun morph |%s|%d|%s|\n", ans_ar_left[0].root, ans_ar_left[0].offset, ans_ar_left[0].pdgm); PRINT_LOG(log_file, logmsg);
				memset(temp_root,'\0',sizeof(temp_root));
				strcpy(temp_root,ans_ar_right[0].root);
				//if(ans_ar_right[0].pdgm !='\0'){//&& ans_ar_left[0].offset==-2) 
				if((ans_ar_right[0].pdgm !='\0')&& (ans_ar_left[0].offset==-2) ){
					//if(strlen(ans_ar_left[0].pdgm)==0) 
					read_clitics ("../../../../data_src/sl/morph/tel/pc_data/clitics-rls", &clitic_pool);
					sprintf(logmsg,"before cli_str in sandhi_splitter test_rgt=|%s|rgt_word1=|%s|ans_ar_right[0].root=|%s|\n",test_right,rgt_word1,ans_ar_right[0].root);PRINT_LOG(log_file,logmsg);
					ret_clitic=clitic_morph(test_right,&clitic_pool,fp_pdgm,DBM_FLAG,db_lex,db_suff,rgt_word1,ans_ar_right,tam,&no_of_ans_clt_rgt);
					sprintf(logmsg,"before cli_str in sandhi_splitter test_rgt=|%s|rgt_word1=|%s|ans_ar_right[0].root=|%s|\n",test_right,rgt_word1,ans_ar_right[0].root);PRINT_LOG(log_file,logmsg);
					if(ret_clitic == 1) {
						//strcpy(tam_clitic,tam); //Nagaraju-070613 for clitic info
						//int flag_unk=2;
						...
						memset(ans_ar_right[0].root,'\0',sizeof(ans_ar_right[0].root));
						memset(ans_ar_right[0].pdgm,'\0',sizeof(ans_ar_right[0].pdgm));
						memset(ans_ar_right[0].aux_verb,'\0',sizeof(ans_ar_right[0].aux_verb));
					}
					strcpy(ans_ar_right[0].root,temp_root);
				}
