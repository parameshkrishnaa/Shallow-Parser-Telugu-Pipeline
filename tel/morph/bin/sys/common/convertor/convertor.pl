	...
	while($line=<STDIN>)
		...
		if(($stype eq "SSF") or ($stype eq "ssf"))
				...
				if($srlang eq "eng")
				{
					...
					{
                        			# backslash incoporated by Rashid to handle punctuations marks '(', ')' etc.
...
{
        $infile =$_[0];
	if ($infile ne "(" and $infile ne ")")  # if statement incorporated by Rashid.
	{
